...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        check_procedure(operator) #checks the operator
        traverse_function = lambda operands: scheme_eval(operands,env)
        if isinstance(operator,MacroProcedure):
            return scheme_eval(operator.apply_macro(rest,env),env)
        return scheme_apply(operator,rest.map(traverse_function),env)
        # END PROBLEM 5
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('too many formals or vals')
        else:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        if target not in env.bindings:
            env.define(target, scheme_eval(expressions.second.first,env))
            return target
        else:
            if env.lookup(target) != scheme_eval(expressions.second.first,env):
                env.define(target, scheme_eval(expressions.second.first,env))
                return target
            else:
        ...
        env.define(target, LambdaProcedure(expressions.first.second,expressions.second,env))
        return target
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        ...
        while expressions.second != nil:
            if scheme_eval(expressions.first,env) == False and str(expressions.first) != '0':
...
def make_let_frame(bindings, env):
    ...
    def function_for_formals_vals(bindings,formals,vals):
        while bindings != nil:
        ...
        return formals,vals
    formals, val = function_for_formals_vals(bindings, nil, nil)
    return env.make_child_frame(formals,val)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        target = expressions.first.first
        env.define(target, MacroProcedure(expressions.first.second,expressions.second,env))
        return target
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
